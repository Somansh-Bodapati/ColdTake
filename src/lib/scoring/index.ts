...
const resolverRegistry = createResolverRegistry([championResolver, topNUnorderedResolver]);
...
function scoreOneAnswer(params: {
  ...
  const { question, resolver, pick, results, boldPickEnabled, boldnessWeight, correctUnitSet, shareForUnit } =
    params;
  ...
  const base = resolver.resolve(question, pick.answer, results);
